...
# import random
import time
from models.phydnet import EncoderRNN, ConvLSTM, PhyCell
# from torch.autograd import Variable
# from penalty import DivergenceLoss
from utils_phydnet import train_epoch, eval_epoch
# , test_epoch
...
phycell = PhyCell(input_shape=(75, 75), input_dim=64, F_hidden_dims=[49], n_layers=1, kernel_size=(7, 7), device=device)
convcell = ConvLSTM(input_shape=(75, 75), input_dim=64, hidden_dims=[128, 128, 64], n_layers=3, kernel_size=(3, 3),
                    device=device)
model = EncoderRNN(phycell, convcell, device=device).to(device)
...
for i in range(50):
    ...
    if valid_mse[-1] < min_mse:
        ...
        torch.save(best_model, "phydnet_model.pth")
    end = time.time()
