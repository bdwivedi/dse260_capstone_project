...
class PhyCell(nn.Module):
    ...
    def forward(self, input_, first_timestep=False, H=None): # input_ [batch_size, 1, channels, width, height]    
        batch_size = input_.data.size()[0]
        if (first_timestep):   
            H = self.initHidden(batch_size) # init Hidden at each forward start
    ...
    def initHidden(self,batch_size):
        ...
        for i in range(self.n_layers):
            H.append( torch.zeros(batch_size, self.input_dim, self.input_shape[0], self.input_shape[1]).to(self.device) )
        return H
...
class ConvLSTM(nn.Module):
    ...
    def initHidden(self,batch_size):
        H, C = [],[]  
        for i in range(self.n_layers):
            H.append( torch.zeros(batch_size,self.hidden_dims[i], self.input_shape[0], self.input_shape[1]).to(self.device) )
            C.append( torch.zeros(batch_size,self.hidden_dims[i], self.input_shape[0], self.input_shape[1]).to(self.device) )
        return H, C
...
class decoder_D(nn.Module):
    def __init__(self, nc=1, nf=32):
        ...
        self.upc3 = nn.ConvTranspose2d(in_channels=nf,out_channels=nc,kernel_size=(3,3),stride=2,padding=1,output_padding=1)  
...
class EncoderRNN(torch.nn.Module):
    ...
    Original: https://github.com/vincent-leguen/PhyDNet
    """
    ...
    def forward(self, input, first_timestep=False, decoding=False, hidden1=None, hidden2=None):
        ...
        hidden1, output1 = self.phycell(input_phys, first_timestep, H=hidden1)
        hidden2, output2 = self.convcell(input_conv, first_timestep, HC=hidden2)

        decoded_Dp = self.decoder_Dp(output1[-1])
        decoded_Dr = self.decoder_Dr(output2[-1])
        
        out_phys = self.activation(self.decoder_D(decoded_Dp))
        out_conv = self.activation(self.decoder_D(decoded_Dr))
                    
        concat = decoded_Dp + decoded_Dr   
        output_image = self.activation( self.decoder_D(concat ))
        return hidden1, hidden2, output_image, out_phys, out_conv