...
#### Code from https://github.com/Rose-STL-Lab/Turbulent-Flow-Net/blob/master/TF-net/model.py
...
def deconv(input_channels, output_channels,kernel_size=4):
...
class LES(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size, dropout_rate, time_range):
        ...
        self.deconv3 = deconv(512, 256,4)
        self.deconv2 = deconv(256, 128,3)
        self.deconv1 = deconv(128, 64)
        self.deconv0 = deconv(64, 32)
        self.output_layer = nn.Conv2d(32 + input_channels, output_channels, kernel_size=kernel_size,padding=(kernel_size - 1) // 2)
        self.activation=torch.sigmoid
        
    def forward(self, xx):
        xx_len = xx.shape[1]
        # u = u_mean + u_tilde + u_prime
        u_tilde = self.spatial_filter(xx.reshape(xx.shape[0]*xx.shape[1], 1, 300, 300)).reshape(xx.shape[0], xx.shape[1], 300, 300)
        # u_prime
        u_prime = (xx - u_tilde)[:,(xx_len - self.input_channels):]
        # u_mean
        u_tilde2 = u_tilde.reshape(u_tilde.shape[0], u_tilde.shape[1],300, 300)
        #print(u_tilde2.shape)
        #print(xx_len//2)
        #print(self.input_channels//2)
        u_mean = []
        for i in range(xx_len - self.input_channels, xx_len):
            '''
            ...
            '''
            cur_mean=self.temporal_filter(u_tilde2[:,i-self.time_range+1:i+1:,:]).unsqueeze(2)
            u_mean.append(cur_mean)
        u_mean = torch.cat(u_mean, dim = 1)
        u_mean = u_mean.reshape(u_mean.shape[0], -1, 300, 300)
        # u_tilde
        u_tilde = u_tilde[:,(self.time_range-1):] - u_mean
       ...
       # print(out_conv2_tilde.shape)
       # print(out_conv2_mean.shape)
        ...
        out = self.activation(self.output_layer(concat0))
       # print(out.shape)
        return out
