...
def train_epoch(train_loader, model, optimizer, loss_function, teacher_force_ratio):
    ...
    for x_fuel,x_windu,x_windv, y_fuel,y_windu,y_windv in train_loader:
        ...
        for i in range(y_fuel.shape[1]):
            ...
            windu=y_windu[:,i]
            windv=y_windv[:,i]
            xx=torch.cat((x_fuel,x_windu,x_windv),dim=1) #b*c*h*w
            im = model(xx)
            '''
            ...
            xx=torch.cat((x_fuel,x_windu,x_windv),dim=1)  
            ...
            if use_teacher_force:
                ...
                x_fuel=torch.cat([x_fuel[:,1:],im[:,0].unsqueeze(1)],1)
                x_windu=torch.cat([x_windu[:,1:],im[:,1].unsqueeze(1)],dim=1)
                x_windv=torch.cat([x_windv[:,1:],im[:,2].unsqueeze(1)],dim=1)
            
            xx=torch.cat((x_fuel,x_windu,x_windv),dim=1)  
            loss += loss_function(im[:,0].unsqueeze(1), y)
            loss+=0.5*loss_function(im[:,1].unsqueeze(1), windu)
            loss+=0.5*loss_function(im[:,2].unsqueeze(1), windv)
    ...
    with torch.no_grad():
        ...
        for x_fuel,x_windu,x_windv, y_fuel,y_windu,y_windv in test_loader:
            ...
            for i in range(y_fuel.shape[1]):
                ...
                x_windv=torch.cat([x_windv[:,1:],im[:,2].unsqueeze(1)],dim=1)
                xx=torch.cat((x_fuel,x_windu,x_windv),dim=1)  
                mse = loss_function(im[:,0].unsqueeze(1), y)
                loss += mse
                loss_curve.append(mse.item())

                ims.append(im[:,0].unsqueeze(1).cpu().data.numpy())
